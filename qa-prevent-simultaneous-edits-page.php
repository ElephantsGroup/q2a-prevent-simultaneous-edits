	...
	public function process_request( $request )
	{
		...
		if((bool)qa_opt('pse_active')) {
			...
			if(QA_FINAL_EXTERNAL_USERS)
				$sql = 'SELECT EP.postid, UNIX_TIMESTAMP(EP.accessed) AS accessed, EP.userid AS user, P.type FROM ^edit_preventer AS EP INNER JOIN ^posts AS P ON EP.postid=P.postid';
			else
				$sql = 'SELECT EP.postid, UNIX_TIMESTAMP(EP.accessed) AS accessed, U.handle AS user, P.type FROM ^edit_preventer AS EP INNER JOIN ^posts AS P ON EP.postid=P.postid
